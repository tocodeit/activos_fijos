<div class="container pt-4">
    <div class="row">
        <div class="col-md-12 text-center">
            <h6>LISTADO DE SOLICITUDES DE PRESTAMOS</h6>
        </div>
        <div class="col-md-12">
            ...
            <div id="btns" class="row">
                <div class="col-md-6">
                    <button id="btn_aceptar" type="button" class="btn btn-sm btn-block btn-outline-primary"
                            onclick="aceptar_solicitud()">Aceptar Solicitud</button>
                </div>
...
<script>
    ...
    function filtro() {
        $.ajax({
            type: 'POST',
            url: baseurl + `api/prestamo/list/2`,
            data: {},
            beforeSend: function() {},
            success: function (res) {
                ...
                res.list.forEach((item, index) => {
                    ...
                    <tr>
                        ...
                        <td width="10%">${item.fecha}</td> <td id="estado_${item.id}">${item.estado}</td> <td>${item.nombre}</td> 
    ...
    function detalle_prestamo(id_prestamo) {
        $.ajax({
            ...
            success: function (res) {
                ...
                if (document.getElementById('estado_'+id_prestamo).innerHTML == 'CONFIRMADO' ||
                        document.getElementById('estado_'+id_prestamo).innerHTML == 'ANULADO') {
    ...
    function aceptar_solicitud() {
        $.ajax({
            type: 'POST',
            url: baseurl + `api/prestamo/update_prestamo/${id_}/${2}`,
            data: {},
            beforeSend: function() {},
            success: function (res) {
                if (console_res) console.log(res);
                ...
                if (res) {
                    document.getElementById('estado_'+id_).innerHTML = 'CONFIRMADO'
                    Swal.fire({title: 'Solicitud aceptada, pendiente para entrega', icon: 'success'})
                    modal.modal('hide')
    ...
    function anular_activos() {
        $.ajax({
            ...
            success: function (res) {
                ...
                const btns = document.getElementById('btns');
                btns.classList.remove('d-none');
                if (document.getElementById('estado_'+id_).innerHTML == 'ENTREGADO' ||
                        document.getElementById('estado_'+id_prestamo).innerHTML == 'ANULADO') {
                    btns.classList.add('d-none');
                }
